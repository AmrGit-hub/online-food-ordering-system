<?php
    ...
    include("rate_class.php");
    $rate = rate::showinstance();
    $user = new user();
    $id = $_SESSION['restaurant_id'];
    $result = $user->showrestaurantinfo($id);
    $restaurant_id = $result['restaurant_id'];
    $restaurant_name = $result['restaurant_name'];
    $restaurant_des = $result['restaurant_des'];
...
<body style="font-family: 'Poppins', sans-serif;">
    ...
    <div class="main_page row w-100 mt-3">
        ...
        <div class="page p-3 pt-1 ps-4 col-9">

            <div class="intro w-100">
                <img class="w-100 rounded-4" style="height:400px;" src="<?php echo $restaurant_image; ?>" alt="">
                <div class="info d-flex flex-row align-items-center pt-3">
                    <h1 class="pe-2"><?php echo $restaurant_name; ?></h1>
                    <h6 style="color:#a8acaf;" class="align-self-end">4.5 <i class="fa-solid fa-star"></i></h6>
                    <h5 style="color:#a8acaf;" class="ms-auto"><?php echo $restaurant_status; ?> Now</h4>
                </div>

                <?php
                    $rate->reseive_rate($restaurant_id);
                ?>
                
                <nav class="post rounded-3 mt-3 d-flex flex-row align-items-center justify-content-between" style="background-color:#edeef0; height:50px;">
                    
                    <h5 class="navbar-text ps-3 pt-3">Navigate Restaurant Items</h5>
                    <h6 style="font-size:13px; color:#a8acaf;" class="pe-4 pt-2"><i class="fa-solid fa-id-badge pe-1"></i> Order id #
                    <?php 
                        $requst = $user->selectorderid();
                        $_SESSION['order_id'] = $requst;
                        echo $requst;
                    ?>
                    </h6>
                </nav>
                ...
                <div class="items mt-4">
                    <?php
                        $user->showallitems($restaurant_id);
                    ?>
                </div>
            </div>
